...
%重排序，eliminator和search algorithm在这里说，full mathch &mismatch intro还是决定在related work
...
%之前已经说到zipper和by是两两相交的特例，只不过使用的search algorithm不同而已,20倍之后才使用binary search
...
\texttt{Adaptive} and \texttt{Sequential} are two prevailing algorithms used for multiple sets intersection, they both use a one-sided binary search of ``galloping" search as F-Search.
...
%\subsubsection{BaezaYates}由于


%这里仅对常用的scalar msis提供一个简单的介绍，for more detailed investigation we would like to recommand that the reads may refer to \cite{}
The Performance of the abovementioned algorithms relies heavily on their searching algorithms.
